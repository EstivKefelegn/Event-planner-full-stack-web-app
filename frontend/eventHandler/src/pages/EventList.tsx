...
function EventList() {
    ...
    const fetchEvents = async () => {
      try {
        const [eventsRes, userRes] = await Promise.all([
          api.get("/"),
          api.get("auth/users/me/"), // Adjust this to match your backend user info route
        ]);
        setEvents(eventsRes.data);
        setUsername(userRes.data.username); // Adjust this key based on API response
      } catch (err) {
        console.error("Failed to fetch events or user", err);
      } finally {
        setLoading(false);
      }
    };
  ...
  return (
    <div className="container">
      ...
      <h1>Upcoming Events {username && <span>for <strong>{username}</strong></span>}</h1>
